...
const CHECKIN_DATE = 'Tue Dec 22 2015 13:45:11 GMT-0500';
...
test('A user can quick rent a car', function(assert) {
  ...
  andThen(function() {
    ...
    assert.equal(rentalItems.length(), 5, 'A user can see only active rentals');
    assert.equal(totalSupply.text(), 10, 'A user can see the total supply for the current fleet');
    assert.equal(availableSupply.text(), 5, 'A user can see the cars available for rent in the current fleet');
